  ...
  return (
    <div className={`min-h-screen ${isDarkMode ? "bg-emerald-950" : "bg-stone-50"} transition-colors duration-300`}>
      ...
      <nav
        ...
        ${isScrolled ? `${isDarkMode ? "bg-emerald-950/80" : "bg-white/80"} backdrop-blur-lg border-b ${isDarkMode ? "border-emerald-800" : "border-gray-100"} shadow-md` : "bg-transparent border-b border-transparent"}`}
      >
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <img src={iconJalur} alt="Logo" className="w-10 h-10" />
              <motion.div className="text-2xl font-bold bg-gradient-to-r from-emerald-600 to-teal-500 bg-clip-text text-transparent" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
                Pacu Jalur
              </motion.div>
            </div>
            ...
            <div className="hidden md:flex items-center space-x-8">
              <Link
                ...
                duration={500}
                offset={-100}
                className={`transition-colors cursor-pointer ${isDarkMode ? "text-stone-300 hover:text-amber-400" : isScrolled ? "text-gray-700 hover:text-amber-600" : "text-white hover:text-amber-300"}`}
              >
                Beranda
              </Link>
              <Link
              ...
              <button onClick={() => setIsDarkMode(!isDarkMode)} className={`p-2 rounded-full ${isDarkMode ? "bg-emerald-800 text-amber-400" : "bg-gray-100 text-gray-600"} hover:scale-110 transition-all`}>
                {isDarkMode ? <Sun className="w-5 h-5" /> : <Moon className="w-5 h-5" />}
              </button>

              <button className="bg-gradient-to-r from-amber-500 to-yellow-600 text-white px-6 py-2 rounded-full hover:shadow-lg transition-all">Beli Tiket</button>
            </div>

            <button className="md:hidden" onClick={() => setIsMenuOpen(!isMenuOpen)}>
              {isMenuOpen ? (
                <X className={`transition-colors ${isDarkMode ? "text-white" : isScrolled ? "text-black" : "text-white"}`} />
              ) : (
                <Menu className={`transition-colors ${isDarkMode ? "text-white" : isScrolled ? "text-black" : "text-white"}`} />
              )}
            </button>
          </div>
        </div>
        ...
        <AnimatePresence>
          {isMenuOpen && (
            <motion.div
              className={`md:hidden ${isDarkMode ? "bg-emerald-900" : "bg-white"} border-t ${isDarkMode ? "border-emerald-800" : "border-gray-200"}`}
              initial={{ opacity: 0, y: -20 }}
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -20 }}
            >
              <div className="px-6 py-4 space-y-4">
                <Link to="home" smooth={true} duration={500} offset={-100} onClick={() => setIsMenuOpen(false)} className={`block ${isDarkMode ? "text-stone-300" : "text-gray-700"} cursor-pointer`}>
                  Beranda
                </Link>
                <Link to="events" smooth={true} duration={500} offset={-80} onClick={() => setIsMenuOpen(false)} className={`block ${isDarkMode ? "text-stone-300" : "text-gray-700"} cursor-pointer`}>
                  Jadwal
                </Link>
                <Link to="gallery" smooth={true} duration={500} offset={-80} onClick={() => setIsMenuOpen(false)} className={`block ${isDarkMode ? "text-stone-300" : "text-gray-700"} cursor-pointer`}>
                  Galeri
                </Link>
                <Link to="lodging" smooth={true} duration={500} offset={-80} onClick={() => setIsMenuOpen(false)} className={`block ${isDarkMode ? "text-stone-300" : "text-gray-700"} cursor-pointer`}>
                  Penginapan
                </Link>
                <Link to="jalur" smooth={true} duration={500} offset={-80} onClick={() => setIsMenuOpen(false)} className={`block ${isDarkMode ? "text-stone-300" : "text-gray-700"} cursor-pointer`}>
                  Jalur
                </Link>
                <button className="w-full bg-gradient-to-r from-amber-500 to-yellow-600 text-white py-2 rounded-full">Beli Tiket</button>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
